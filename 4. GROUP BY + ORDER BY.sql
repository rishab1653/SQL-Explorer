...
INSERT INTO employe (EmployeeID, FirstName, LastName, Gender, DateOfBirth, DepartmentID, Salary, ManagerID, HireDate, Address, Email)
VALUES
...
SELECT GENDER FROM EMPLOYE;
-- GROUP BY :- GROUP TOGETHER ROWS THAT HAS SAME VALUE IN SPECIFIED COLUMNS
SELECT GENDER
FROM EMPLOYE
GROUP BY GENDER;


SELECT GENDER, AVG(SALARY) AS AVERAGE_SALARY , MAX(SALARY) AS MINIMUM_SALARY , MIN(SALARY) AS MAXIMUM_SALARY , COUNT(SALARY) AS TOTAL_COUNT
FROM EMPLOYE
GROUP BY GENDER;


--  ORDER BY :- IT IS ACTUALLY GOING TO SORT THE RESULT SET IN EITHER ASCENDING(ASC) OR DESCENDING(DESC)
-- BUT BY DEFAULT IT IS ASCENDING 
...
SELECT * 
FROM EMPLOYE
ORDER BY 4, 7;  -- 4 MEAANS GENDER AND 7 MEANS SALARY INSTEAD OF WRITE NAME JUST GIVE NUMBER OF THAT PARTICULAR COLUMN
